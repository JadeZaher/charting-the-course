...
interface CatalogueProps {
  search?: string;
  onBrowseEcosystems: () => void;
  onBrowseExchanges: () => void;
}

function EcosystemSharesNeedsSection({ ecosystem, onBrowseExchanges }: { ecosystem: EthosSummary; onBrowseExchanges: () => void }) {
  ...
  return (
    <section className="border-2 border-strong-border bg-card" aria-labelledby={`exchange-${ecosystem.id}`}>
      ...
      <div className="border-t-2 border-strong-border p-4 text-right">
        <button
          type="button"
          onClick={onBrowseExchanges}
          className="inline-flex min-h-11 items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-link hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
...
// Solutions catalogue merged from the former /solutions page (pages/Discover.tsx).
// Browsing stays driven by ecosystem selection; admins may scope to one ecosystem.
export default function SolutionsCatalogue({ search = '', onBrowseEcosystems, onBrowseExchanges }: CatalogueProps) {
  ...
  const scopedEthos = permissions.isAdmin
    ? (adminSelectedEthosId ? allEthos.filter((ethos) => ethos.id === adminSelectedEthosId) : allEthos)
    : accessibleEthos;

  // Hub search applies client-side to the portfolio; clearing it restores the full scope.
  const queryText = search.trim().toLowerCase();
  const displayEthos = queryText
    ? scopedEthos.filter((ethos) =>
        [ethos.name, ethos.tagline, ethos.description, ethos.location, ethos.sector, ...(ethos.tags ?? [])]
  ...
  if (!permissions.isAdmin && accessibleEthos.length === 0) {
    return (
      <div className="border-2 border-dashed border-strong-border p-10 sm:p-16">
        ...
        <button
          type="button"
          onClick={onBrowseEcosystems}
          className="mt-6 inline-flex min-h-11 items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-link hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
          Browse ecosystems <ArrowRight className="h-4 w-4" aria-hidden="true" />
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-12">
      <div className="grid gap-5 border-b-2 border-strong-border pb-6 lg:grid-cols-12 lg:items-end">
        <div className="lg:col-span-7">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-muted-foreground">Personal index</p>
          <h2 className="mt-2 text-3xl font-black tracking-[-0.035em]">My solutions.</h2>
        ...
        {permissions.isAdmin && allEthos.length > 1 && (
          <div className="lg:col-span-4 lg:col-start-9">
            <label htmlFor="solution-filter" className="mb-2 block text-[0.68rem] font-black uppercase tracking-[0.14em]">Admin view</label>
      ...
      <section aria-labelledby="solution-portfolio-title" className="space-y-5">
        ...
        {displayEthos.length === 0 ? (
          <div className="border-2 border-dashed border-strong-border p-10 sm:p-16">
            ...
            <p className="mt-3 text-2xl font-black tracking-tight">No solutions match the current search.</p>
            <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Clear the search or broaden the ecosystem selection to restore the portfolio.</p>
          </div>
        ) : (
          <div className="grid gap-5 lg:grid-cols-2">
      ...
      {displayEthos.length > 0 && (
        <section aria-labelledby="exchange-index-title" className="space-y-5">
          ...
          {displayEthos.map((ethos) => <EcosystemSharesNeedsSection key={ethos.id} ecosystem={ethos} onBrowseExchanges={onBrowseExchanges} />)}
        </section>
      )}
